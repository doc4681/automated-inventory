...
COST_THRESHOLD_LOW = 10.0   # Sotto 10.00
MARKUP_LOW = 2.2            # Markup 2.2
COST_THRESHOLD_MID = 20.0   # Sotto 20.00
MARKUP_MID = 1.9            # Markup 1.9
...
def check_brand_compatibility(shopify_tags, supplier_brand):
    if pd.isna(supplier_brand) or not str(supplier_brand).strip(): return True 
    if pd.isna(shopify_tags) or not str(shopify_tags).strip(): return True 
    ...
    for tag in tags_list:
        ...
        if norm_supplier == norm_tag or norm_supplier == norm_tag_clean: return True
        if norm_supplier in norm_tag: return True
    return False
...
def load_markup_rules(markup_file_content):
    ...
    try:
        if not hasattr(markup_file_content, 'read'): return markup_dict
...
def load_trademarks(file_obj):
    ...
    try:
        if hasattr(file_obj, 'read'):
            ...
            for line in lines:
                tm = line.strip()
                if tm: trademarks.add(normalize_string(tm))
    except: pass
    return trademarks
...
def calculate_target_price_and_markup(cost, tags, supplier_brand, markup_rules, valid_trademarks_normalized):
    ...
    # 3. Regola Brand Valido (Se costo >= 20)
        ...
        if is_valid_brand:
            ...
            # Se stiamo facendo sync normale e abbiamo trovato fornitore (supplier_brand non vuoto),
            # usiamo un default. Se siamo in Markup Only (supplier_brand=None) e il brand non è valido, saltiamo.
            if supplier_brand:
                markup = 1.50
...
def process_inventory_v03(df_shopify, df_mcws, df_bbr, markup_file, valid_trademarks_file, include_change_log=True, only_changes=True, enable_bbr=True):
    ...
    stats = {'processed': 0, 'updated_qty': 0, 'updated_cost': 0, 'updated_price': 0, 'errors': 0, 'inventory': {'total': 0, 'updates_1': 0, 'updates_0': 0}}
    logs = []
    
    output_df = df_shopify.copy()
    if COL_CHANGE_LOG not in output_df.columns: output_df[COL_CHANGE_LOG] = ''
    if COL_COMPARE not in output_df.columns: output_df[COL_COMPARE] = ''
    ...
    for index, row in output_df.iterrows():
        try:
            ...
            # B. CHECK BBR ORFANO (Solo se BBR abilitato)
    ...
    if not include_change_log: final_df = final_df.drop(columns=[COL_CHANGE_LOG], errors='ignore')
...
def process_markup_only(df_shopify, markup_file, valid_trademarks_file):
    ...
    for index, row in output_df.iterrows():
        try:
            ...
            # USA HELPER CENTRALIZZATO (supplier_brand=None)
            target_price, markup, reason = calculate_target_price_and_markup(
